...
// guaranteed to run first. See `AnalyticsProvider.test.tsx` for the pattern.
//
// Deliberately duplicated (not shared cross-package) from
// `packages/react/src/testSetup.ts` -- see this issue's plan doc, which
// re-affirms issue 001's Context on this point.
