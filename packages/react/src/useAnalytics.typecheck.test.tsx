...
// never invoked (it can't be: it calls a hook, and there is no surrounding
// render/component tree here). `bun run typecheck` / `typecheck:tsc`
// (already run in CI) are what actually exercise this file: a regression in
// `useAnalytics`'s generic plumbing surfaces as a type error there, not as a
// failing runtime assertion in this file. No dedicated type-testing harness
// exists in this repo yet -- introducing one is out of scope for this
// issue.
//
...
// Named with a `use` prefix so this reads as (and lints as) a custom hook,
// even though it is never actually called.
